...
function getRandomfandomOpts() {
  $randomfandomOpts = array(
...
function alterAdmin($args) {
  ...
  # Place a label identifying these are Random Fanodm theme options
  ...
  if (acGet('source') === 'config.txt') {
    ...
    $vals_html = "<p><ul>\n";
    $randomfandomOpts = getRandomfandomOpts();
    foreach ($randomfandomOpts as $id => $label) {
  ...
  # Loop over the getRandomfandomOpts() and made input fields for each one.
  $randomfandomOpts = getRandomfandomOpts();
  foreach ($randomfandomOpts as $id => $label) {
    # Field label
    $real_id = preg_replace('/^theme:/', '', $id);
...
$RandomfandomConf = loadRandomfandomConf();

# This function loads the global $RandomfandomConf, preferentially from
# from config.txt if it exits, and if not, from wCMS settings that
# the theme adds.
function loadRandomfandomConf() {
  global $Wcms;
...
# Gets an $randomfandomConf value or empty string if it does not exist
function acGet($key) {
  global $RandomfandomConf;
  if (! (array_key_exists('loaded', $RandomfandomConf) and $RandomfandomConf['loaded'])) {
    $RandomfandomConf = loadRandomfandomConf();
  }
  if (array_key_exists($key, $RandomfandomConf)) {
    return $RandomfandomConf[$key];
  }
  return '';
}

# Returns the HTML for the footer's LinkedIn link.
# Uses $RandomfandomConf settings from theme's config.txt
...
# The $Wcms->menu() hook for the Random Fandom theme.
# $Wcms->addListener('menu', 'getMenuRandomfandom');
function getMenuRandomfandom($args) {
  global $Wcms;
  $output = '';
